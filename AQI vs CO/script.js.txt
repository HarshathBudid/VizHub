...
svg
  ...
  .text('AQI value vs CO AQI Value'); // Chart title
...
d3.csv(
  ...
  data.forEach((d) => {
    ...
    d.CO_AQI_Value = +d['CO AQI Value'];
  });

  // Create scales
  const xScale = d3
    .scaleLinear()
    .domain([0, d3.max(data, (d) => d.CO_AQI_Value)])
    .range([0, chartWidth]);
  ...
  chart
    ...
    .attr('fill', 'black')
    .text('CO AQI Value'); // Label for x-axis
  ...
  chart
    ...
    .enter()
    .append('circle')
    .attr('cx', (d) => xScale(d.CO_AQI_Value)) // X-axis position
