...
svg
  ...
  .text('AQI value vs Ozone AQI Value'); // Chart title
...
d3.csv(
  ...
  data.forEach((d) => {
    ...
    d.Ozone_AQI_Value = +d['Ozone AQI Value'];
  });

  // Create scales
  const xScale = d3
    .scaleLinear()
    .domain([0, d3.max(data, (d) => d.Ozone_AQI_Value)])
    .range([0, chartWidth]);
  ...
  chart
    ...
    .attr('fill', 'black')
    .text('Ozone AQI Value'); // Label for x-axis
  ...
  chart
    ...
    .enter()
    .append('circle')
    .attr('cx', (d) => xScale(d.Ozone_AQI_Value)) // X-axis position
