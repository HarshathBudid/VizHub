...
svg
  ...
  .text('AQI value vs PM2.5 AQI Value'); // Chart title
...
d3.csv(
  ...
  data.forEach((d) => {
    ...
    d.PM2_AQI_Value = +d['PM2.5 AQI Value'];
  });

  // Create scales
  const xScale = d3
    .scaleLinear()
    .domain([0, d3.max(data, (d) => d.PM2_AQI_Value)])
    .range([0, chartWidth]);
  ...
  chart
    ...
    .attr('fill', 'black')
    .text('PM2.5 AQI Value'); // Label for x-axis
  ...
  chart
    ...
    .enter()
    .append('circle')
    .attr('cx', (d) => xScale(d.PM2_AQI_Value)) // X-axis position
